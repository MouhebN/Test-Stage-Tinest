...
import ModifierColisModal from './ModifierColisModal';
...
function App() {
  ...
  const fetchData = () => {
    axios
      .get('http://localhost:3000/colis/listerColis')
      .then((response) => {
  ...
  const ajouterColis = async () => {
    try {
      if (window.confirm("Voulez-vous vraiment ajouter ce colis ?")) {
        await axios.post('http://localhost:3000/colis/ajouterColis', nouveauColis);
  ...
  const handleSupprimer = async (colisId) => {
    ...
    if (shouldDelete) {
      try {
        await axios.delete(`http://localhost:3000/colis/${colisId}/supprimerColis`);
        fetchData();
      } catch (error) {
  ...
  const handleSubmitModification = async (editedColis) => {
    try {
      const response = await axios.post(
        `http://localhost:3000/colis/${editedColis._id}/modifierColis`,
        editedColis
      );
